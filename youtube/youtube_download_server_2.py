...
class YoutubeServerTwo:
    ...
    def get_channel_id(self):
        ...
        match = re.search(r"@([\w-]+)", self.option.channel_url)
        if not match:
            return None
        channel_name = match.group(1)

        # Gọi API để lấy ID kênh
        url = f"https://www.googleapis.com/youtube/v3/search?part=id&q={channel_name}&type=channel&key={self.option.API_KEY}"
    ...
    def get_video_details(self,video_ids):
        ...
        url = f"https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id={','.join(video_ids)}&key={self.option.API_KEY}"
        response = requests.get(url).json()
    ...
    def get_videos(self,channel_id):
        ...
        order = order_mapping.get(int(self.option.video_sort), "date")
        video_list = []
        next_page_token = ""
        
        while True:
            # 🔹 Gọi API lấy video
            url = f"https://www.googleapis.com/youtube/v3/search?part=snippet&channelId={channel_id}&key={self.option.API_KEY}&order={order}&maxResults=50&type=video&pageToken={next_page_token}"
            response = requests.get(url).json()
            ...
            for video_id in video_ids:
                ...
                if self.option.video_duration == YoutubeKey.VideoDuaration.VIDEODURATION_FROM1TO4MINUTE and duration >= 240:
                    continue  # Loại bỏ video dài hơn 4 phút
                if self.option.video_duration == YoutubeKey.VideoDuaration.VIDEODURATION_UNDER1MINUTE and duration >= 60:
                    continue  # Loại bỏ video không phải Shorts

                video_list.append(video_id)

            # 🔹 Nếu chỉ lấy số lượng nhất định, dừng khi đủ
            if int(self.option.count) > 0 and len(video_list) >= int(self.option.count):
                break

            # 🔹 Nếu có trang tiếp theo, tiếp tục lấy
            next_page_token = response.get("nextPageToken", "")
            if not next_page_token or int(self.option.count) == -1:
                break

        return video_list[0:int(self.option.count)] if int(self.option.count) == -1 else video_list
    ...
    def download_video_and_audio(self,video_id):
        """Tải video từ YouTube và ghép âm thanh."""
        url = f'https://youtube.com/watch?v={video_id}'
        # url = f'https://www.youtube.com/shorts/{video_id}'

        resolution = "1080p" if self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[0] or self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[2] else "360p"

        try:
            yt = YouTube(url)
            safe_title = self.sanitize_filename(yt.title)
            output_file = os.path.join(self.option.output_path, f"{safe_title}.mp4")
            ...
            if video_stream and audio_stream:
                video_file = ""
                audio_file = ""
                if self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[0]:
                ...
                if self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[2] or self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[4]:
                    video_file = video_stream.download(output_path=self.VIDEO_TEMP_PATH, filename='video.mp4')
                if self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[3] or self.option.video_quatity == YoutubeKey.VideoQuality.VideoFormat[5]:
                    audio_file = audio_stream.download(output_path=self.VIDEO_TEMP_PATH, filename='audio.mp4')
            else:
    ...
    def run(self):
        ...
        if not channel_id:
            ...
            video_list = self.get_videos(channel_id)
